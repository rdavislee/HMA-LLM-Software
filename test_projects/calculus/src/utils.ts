...
const op = (operator: BinaryOperatorType, left: ExpressionNode, right: ExpressionNode) => new BinaryOperationNode(operator, left, right);
    ...
    ExpressionSimplifier
{
    ...
    differentiate(expression: ExpressionNode, targetVariable: string): ExpressionNode {
        ...
        switch (expression.type) {
            ...
            case 'functionCall':
                ...
                switch (funcCallExpr.name) {
                    case 'sin': result = op('multiply', func('cos', [arg]), darg); break;
                    case 'cos': result = op('multiply', num(-1), op('multiply', func('sin', [arg]), darg)); break;
                    ...
                    case 'exp': result = op('multiply', new ExponentialNode(arg), darg); break; // Changed func('exp', [arg]) to new ExponentialNode(arg)
            ...
            case 'unaryOperation': // NEW
                ...
                result = new UnaryOperationNode(unOpExpr.operator, dOperand); // d(-u)/dx = -du/dx
                break;
            case 'constant': // NEW
    ...
    integrateIndefinite(expression: ExpressionNode, targetVariable: string): IntegrationResult {
        ...
        let integratedExpr: ExpressionNode | "UNINTEGRATABLE_EXPRESSION" = "UNINTEGRATABLE_EXPRESSION";
        let unintegratable = true;
        let constantOfIntegration: string | null = null;

        switch (expression.type) {
            case 'number':
                const numExpr = expression as NumberNode;
                // ∫c dx = cx + C
                integratedExpr = op('add', op('multiply', numExpr, variable(targetVariable)), new VariableNode('C'));
            ...
            case 'variable':
                ...
                if (varExpr.name === targetVariable) {
                    ...
                    integratedExpr = op('add', op('divide', op('power', variable(targetVariable), num(2)), num(2)), new VariableNode('C'));
                    unintegratable = false;
                    constantOfIntegration = 'C';
                } else {
                    // ∫y dx = yx + C (if y is treated as a constant wrt x)
                    integratedExpr = op('add', op('multiply', varExpr, variable(targetVariable)), new VariableNode('C'));
                    unintegratable = false;
                    constantOfIntegration = 'C';
                }
                break;
            case 'binaryOperation':
                ...
                if (binOpExpr.operator === 'add' || binOpExpr.operator === 'subtract') {
                    ...
                    if (!leftResult.unintegratable && !rightResult.unintegratable) {
                        integratedExpr = op('add', op(binOpExpr.operator, leftResult.integratedExpression as ExpressionNode, rightResult.integratedExpression as ExpressionNode), new VariableNode('C'));
                        unintegratable = false;
                        constantOfIntegration = 'C';
                    }
                    ...
                    if (n === -1) { // ∫1/x dx = ln(|x|) + C
                        integratedExpr = op('add', func('ln', [func('abs', [variable(targetVariable)])]), new VariableNode('C'));
                        unintegratable = false;
                        constantOfIntegration = 'C';
                    } else {
                        integratedExpr = op('add', op('divide', op('power', variable(targetVariable), num(n + 1)), num(n + 1)), new VariableNode('C'));
                        unintegratable = false;
                        constantOfIntegration = 'C';
                    }
                } else if (binOpExpr.operator === 'power' && binOpExpr.right.type === 'variable' && (binOpExpr.right as VariableNode).name === targetVariable && binOpExpr.left.type === 'number') {
                    // ∫a^x dx = a^x / ln(a) + C
                    const a = (binOpExpr.left as NumberNode).value;
                    if (a <= 0) { // log(a) undefined or complex for a<=0
                        ...
                        integratedExpr = op('add', variable(targetVariable), new VariableNode('C'));
                        unintegratable = false;
                        constantOfIntegration = 'C';
                    } else {
                        integratedExpr = op('add', op('divide', expression, func('ln', [num(a)])), new VariableNode('C'));
                        unintegratable = false;
                        constantOfIntegration = 'C';
                    }
                }
            ...
            case 'functionCall':
                ...
                if (funcCallExpr.args.length === 1 && funcCallExpr.args[0].type === 'variable' && (funcCallExpr.args[0] as VariableNode).name === targetVariable) {
                    switch (funcCallExpr.name) {
                        case 'sin':
                            integratedExpr = op('add', op('multiply', num(-1), func('cos', [variable(targetVariable)])), new VariableNode('C'));
                            unintegratable = false;
                            constantOfIntegration = 'C';
                            break;
                        case 'cos':
                            integratedExpr = op('add', func('sin', [variable(targetVariable)]), new VariableNode('C'));
                            unintegratable = false;
                            constantOfIntegration = 'C';
                            break;
                        case 'exp':
                            integratedExpr = op('add', new ExponentialNode(variable(targetVariable)), new VariableNode('C'));
                            unintegratable = false;
                            constantOfIntegration = 'C';
                            break;
                        case 'ln': // ∫ln(x)dx = xln(x) - x + C
                            integratedExpr = op('add', op('subtract', op('multiply', variable(targetVariable), func('ln', [variable(targetVariable)])), variable(targetVariable)), new VariableNode('C'));
                            unintegratable = false;
                            constantOfIntegration = 'C';
                            break;
                    }
            ...
            case 'unaryOperation': // NEW
                ...
                if (unOpExpr.operator === 'negate') {
                    ...
                    if (!result.unintegratable) {
                        integratedExpr = op('add', new UnaryOperationNode('negate', result.integratedExpression as ExpressionNode), new VariableNode('C'));
                        unintegratable = false;
                        constantOfIntegration = 'C';
                    }
                }
            ...
            case 'constant': // NEW
                const constExpr = expression as ConstantNode;
                // ∫c dx = cx + C
                integratedExpr = op('add', op('multiply', constExpr, variable(targetVariable)), new VariableNode('C'));
                unintegratable = false;
                constantOfIntegration = 'C';
                break;
            case 'exponential': // NEW
                ...
                if (expExpr.exponent.type === 'variable' && (expExpr.exponent as VariableNode).name === targetVariable) {
                    integratedExpr = op('add', expExpr, new VariableNode('C'));
                    unintegratable = false;
                    constantOfIntegration = 'C';
                }
                break;
    ...
    integrateDefinite(expression: ExpressionNode, targetVariable: string, lowerBound: number, upperBound: number, options?: IntegrationOptions): number {
        ...
        if (numRectangles && numRectangles > 0) {
            ...
            const antiderivative = indefiniteResult.integratedExpression as ExpressionNode;
            ...
            try {
                ...
                const contextUpper: EvaluationContext = { [targetVariable]: upperBound, 'C': 0 };
                const contextLower: EvaluationContext = { [targetVariable]: lowerBound, 'C': 0 };
        ...
        switch (expression.type) {
            ...
            case 'binaryOperation':
                ...
                if (simplifiedLeft.type === 'number' && simplifiedRight.type === 'number') {
                    ...
                    switch (binOpExpr.operator) {
                        ...
                        case 'divide':
                            if (rightVal === 0) throw new Error("Division by zero during simplification.");
                            return num(leftVal / rightVal);
                        case 'power':
                            if (leftVal === 0 && rightVal === 0) throw new Error("0^0 is undefined."); // Added 0^0 check
                ...
                if (binOpExpr.operator === 'add') {
                    if (simplifiedLeft.type === 'number' && (simplifiedLeft as NumberNode).value === 0) return simplifiedRight;
                    if (simplifiedRight.type === 'number' && (simplifiedRight as NumberNode).value === 0) return simplifiedLeft;
                    ...
                    if (simplifiedLeft.type === 'binaryOperation' && (simplifiedLeft as BinaryOperationNode).operator === 'multiply' &&
                        (simplifiedLeft as BinaryOperationNode).left.type === 'number' &&
                        ...
                        (simplifiedLeft as BinaryOperationNode).right.type === 'variable' && (simplifiedRight as BinaryOperationNode).right.type === 'variable' &&
                        ...
                        return op('multiply', num(((simplifiedLeft as BinaryOperationNode).left as NumberNode).value + ((simplifiedRight as BinaryOperationNode).left as NumberNode).value), variable(((simplifiedLeft as BinaryOperationNode).right as VariableNode).name));
                    }
                    ...
                    if (simplifiedLeft.type === 'variable' && simplifiedRight.type === 'variable' && (simplifiedLeft as VariableNode).name === (simplifiedRight as VariableNode).name) {
                        return op('multiply', num(2), variable((simplifiedLeft as VariableNode).name));
                    }
                    ...
                    if (simplifiedLeft.type === 'variable' &&
                        ...
                        (simplifiedLeft as VariableNode).name === ((simplifiedRight as BinaryOperationNode).right as VariableNode).name) {
                        return op('multiply', num(1 + ((simplifiedRight as BinaryOperationNode).left as NumberNode).value), variable((simplifiedLeft as VariableNode).name));
                    }
                    ...
                    if (simplifiedRight.type === 'variable' &&
                        ...
                        (simplifiedRight as VariableNode).name === ((simplifiedLeft as BinaryOperationNode).right as VariableNode).name) {
                        return op('multiply', num(((simplifiedLeft as BinaryOperationNode).left as NumberNode).value + 1), variable((simplifiedRight as VariableNode).name));
                    }
                }
                if (binOpExpr.operator === 'subtract') {
                    ...
                    if (simplifiedLeft.type === 'number' && (simplifiedLeft as NumberNode).value === 0) {
                        return new UnaryOperationNode('negate', simplifiedRight);
                    }
                }
                if (binOpExpr.operator === 'multiply') {
                    ...
                    if (simplifiedRight.type === 'number' && (simplifiedRight as NumberNode).value === -1) {
                        return new UnaryOperationNode('negate', simplifiedLeft);
                    }
                ...
                if (binOpExpr.operator === 'divide') {
                    if (simplifiedRight.type === 'number' && (simplifiedRight as NumberNode).value === 1) return simplifiedLeft;
                    if (simplifiedLeft.type === 'number' && (simplifiedLeft as NumberNode).value === 0) return num(0); // 0/x = 0
                    ...
                    if (simplifiedLeft.type === 'number' &&
                        simplifiedRight.type === 'binaryOperation' && (simplifiedRight as BinaryOperationNode).operator === 'multiply' &&
                        (simplifiedRight as BinaryOperationNode).left.type === 'number' && (simplifiedLeft as NumberNode).value === ((simplifiedRight as BinaryOperationNode).left as NumberNode).value) {
                        return op('divide', num(1), (simplifiedRight as BinaryOperationNode).right);
                    }
                    // Simplify (C*X) / C = X
                    if (simplifiedLeft.type === 'binaryOperation' && (simplifiedLeft as BinaryOperationNode).operator === 'multiply' &&
                ...
                if (binOpExpr.operator === 'power') {
                    if (simplifiedRight.type === 'number' && (simplifiedRight as NumberNode).value === 0) return num(1); // x^0 = 1
                    if (simplifiedRight.type === 'number' && (simplifiedRight as NumberNode).value === 1) return simplifiedLeft; // x^1 = x
                    if (simplifiedLeft.type === 'number' && (simplifiedLeft as NumberNode).value === 0) { // 0^x = 0 (for x > 0)
                        if (simplifiedRight.type === 'number' && (simplifiedRight as NumberNode).value <= 0) throw new Error("0^0 is undefined or 0^negative is infinity."); // 0^0 handled above
                        return num(0);
                    }
                    if (simplifiedLeft.type === 'number' && (simplifiedLeft as NumberNode).value === 1) return num(1); // 1^x = 1
            ...
            case 'functionCall':
                ...
                if (simplifiedArgs.length === 1 && simplifiedArgs[0].type === 'number') {
                    const argVal = (simplifiedArgs[0] as NumberNode).value;
                    switch (funcCallExpr.name) {
                        ...
                        case 'ln':
                            if (argVal === 1) return num(0); // ln(1) = 0
                            if (argVal > 0) return num(Math.log(argVal));
                            break; // Cannot simplify ln(0) or ln(negative)
                        case 'exp':
                            if (argVal === 0) return num(1); // exp(0) = 1
                            return num(Math.exp(argVal));
                        case 'sqrt':
                            if (argVal === 0) return num(0); // sqrt(0) = 0
                ...
                // Simplify ln(e) = 1
                if (funcCallExpr.name === 'ln' && simplifiedArgs.length === 1 && simplifiedArgs[0].type === 'constant' && (simplifiedArgs[0] as ConstantNode).name === 'e') {
                    return num(1);
                }
                // Simplify exp(ln(x)) = x
                if (funcCallExpr.name === 'exp' && simplifiedArgs.length === 1 && simplifiedArgs[0].type === 'functionCall' && (simplifiedArgs[0] as FunctionCallNode).name === 'ln' && (simplifiedArgs[0] as FunctionCallNode).args.length === 1) {
                    return (simplifiedArgs[0] as FunctionCallNode).args[0];
                }
            ...
            case 'unaryOperation': // NEW
                ...
                if (simplifiedOperand.type === 'number') {
                    switch (unOpExpr.operator) {
                        case 'negate': return num(-((simplifiedOperand as NumberNode).value));
                    }
                }
                // Simplify -(-x) = x
                if (unOpExpr.operator === 'negate' && simplifiedOperand.type === 'unaryOperation' && (simplifiedOperand as UnaryOperationNode).operator === 'negate') {
