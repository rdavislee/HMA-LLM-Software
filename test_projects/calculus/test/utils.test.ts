...
import { ExpressionEvaluator, ExpressionDifferentiator, ExpressionIntegrator, ExpressionSimplifier, EvaluationContext } from '../src/utils.interface';
import {
    ExpressionNode,
    BinaryOperatorType,
    UnaryOperatorType,
...
import {
    ...
    ConstantNode,
    ExponentialNode
} from '../src/expression'; // Import concrete classes
import { ExpressionUtils } from '../src/utils';

// Assign the actual evaluators from expressionUtils
const utilsInstance = new ExpressionUtils();
const evaluator: ExpressionEvaluator = utilsInstance;
const differentiator: ExpressionDifferentiator = utilsInstance;
const integrator: ExpressionIntegrator = utilsInstance;
const simplifier: ExpressionSimplifier = utilsInstance; // Add simplifier

// Helper function to create literal nodes using concrete classes
const num = (value: number): NumberNode => new NumberNode(value);
const variable = (name: string): VariableNode => new VariableNode(name);

// Helper for binary operations using concrete classes
const op = (left: ExpressionNode, operator: BinaryOperatorType, right: ExpressionNode): BinaryOperationNode => new BinaryOperationNode(operator, left, right);

// Helper for unary operations (e.g., negation) using concrete classes
const unaryOp = (operator: UnaryOperatorType, operand: ExpressionNode): UnaryOperationNode => new UnaryOperationNode(operator, operand);

// Helper for function calls using concrete classes
const func = (name: FunctionNameType, args: ExpressionNode[]): FunctionCallNode => new FunctionCallNode(name, args);

// Add a helper for deep comparison of expression nodes after simplification
const expectSimplified = (input: ExpressionNode, expected: ExpressionNode) => {
    const simplified = simplifier.simplify(input);
    expect(simplified).to.deep.equal(expected);
};
...
describe('ExpressionEvaluator', () => {
    ...
    describe('evaluate()', () => {
        ...
        it('should handle negative numbers', () => {
            const expr = op(num(-5), 'add', num(3));
            expect(evaluator.evaluate(expr, {})).to.equal(-2);
        });
        ...
        it('should correctly evaluate nested functions', () => {
            ...
            expect(evaluator.evaluate(expr, {})).to.be.closeTo(Math.sin(1), 1e-9);
        });

        it('should handle unary negation', () => {
            const expr = unaryOp('negate', num(5)); // Represents -5
            expect(evaluator.evaluate(expr, {})).to.equal(-5);
        });
        ...
        it('should throw error for division by zero', () => {
            ...
            expect(() => evaluator.evaluate(expr, {})).to.throw('Division by zero.');
        });

        it('should throw error for log of non-positive number', () => {
            const expr = func('ln', [num(0)]);
            expect(() => evaluator.evaluate(expr, {})).to.throw('Logarithm of non-positive number.');
        });
        ...
        it('should handle complex nested expressions', () => {
            ...
            const expr = op(
                op(
                    op(num(2), 'multiply', func('sin', [op(num(Math.PI), 'divide', num(6))])),
                    'add',
                    func('ln', [op(num(Math.E), 'power', num(2))])
                ),
                'subtract',
                op(op(num(4), 'subtract', variable('x')), 'power', num(2))
            );
            expect(evaluator.evaluate(expr, { x: 1 })).to.be.closeTo(-6, 1e-9);
        });
    });
});
...
describe('ExpressionDifferentiator', () => {
    ...
    describe('differentiate()', () => {
        it('should differentiate a constant to 0', () => {
            ...
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(expected);
        });
        ...
        it('should apply product rule: d/dx(x * sin(x)) = sin(x) + x*cos(x)', () => {
            ...
            const expected = op(
                func('sin', [variable('x')]),
                'add',
                op(variable('x'), 'multiply', func('cos', [variable('x')]))
            );
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(expected);
        });
        ...
        it('should differentiate tan(x) to sec^2(x) (or 1/cos^2(x))', () => {
            ...
            const expected = op(
                num(1),
                'divide',
                op(func('cos', [variable('x')]), 'power', num(2))
            );
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(expected);
        });
        ...
        it('should apply chain rule: d/dx(sin(x^2)) = cos(x^2) * 2x', () => {
            ...
            const expected = op(
                func('cos', [innerExpr]),
                'multiply',
                op(num(2), 'multiply', variable('x')) // 2x
            );
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(expected);
        });
        ...
        it('should handle complex expression: d/dx(x^2 + 3x - 5) = 2x + 3', () => {
            const expr = op(
                op(op(variable('x'), 'power', num(2)), 'add', op(num(3), 'multiply', variable('x'))),
                'subtract',
                num(5)
            );
            const expected = op(
                op(num(2), 'multiply', variable('x')),
                'add',
                num(3)
            );
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(expected);
        ...
        it('should handle differentiation of an expression with respect to a different variable', () => {
            ...
            const expected = num(0);
            expect(differentiator.differentiate(expr, 'y')).to.deep.equal(expected);
        });
        ...
        it('should differentiate e^x to e^x', () => {
            ...
            const expected = new ExponentialNode(variable('x'));
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(expected);
        });
        ...
        it('should simplify x+x to 2x before differentiation: d/dx(x+x) = 2', () => {
            ...
            expect(differentiator.differentiate(expr, 'x')).to.deep.equal(num(2)); // d/dx(2x) = 2
        });
    });
});
...
describe('ExpressionSimplifier', () => {
    ...
    describe('simplify()', () => {
        ...
        it('should handle division by zero during constant folding', () => {
            ...
            expect(simplified).to.deep.equal(expr); // Simplifier returns original node for division by zero
            expect(() => evaluator.evaluate(simplified, {})).to.throw('Division by zero.');
        });
        ...
        it('should simplify ln(e) to 1', () => {
            const expr = func('ln', [new ConstantNode('e')]);
            const expected = num(1);
            expectSimplified(expr, expected);
        });
        ...
        it('should apply multiple simplifications in a nested expression: (x+0)*1^1 - (y-y)', () => {
            ...
            const expr = op(
                op(
                    op(variable('x'), 'add', num(0)),
                    'multiply',
                    op(num(1), 'power', num(1))
                ),
                'subtract',
                op(variable('y'), 'subtract', variable('y'))
            );
            const expected = variable('x');
            expectSimplified(expr, expected);
        });
        ...
        it('should simplify expressions involving PI and E constants', () => {
            ...
            const expr = op(
                op(
                    op(new ConstantNode('pi'), 'multiply', num(2)),
                    'multiply',
                    new ConstantNode('e')
                ),
                'add',
            ...
            const expected = op(
                op(num(2), 'multiply', new ConstantNode('pi')),
                'multiply',
                new ConstantNode('e')
            );
            expectSimplified(expr, expected);
        ...
        it('should simplify 0^0 if it throws (or is handled as NaN/undefined)', () => {
            ...
            expect(() => simplifier.simplify(expr)).to.throw('0^0 is undefined.');
        });
...
describe('ExpressionIntegrator', () => {
    ...
    const checkIndefiniteResult = (
        ...
        if (expectedUnintegratable) {
            ...
            expect(result.integratedExpression).to.deep.equal(expectedIntegratedExpr);
        }
    };
    ...
    describe('integrateIndefinite()', () => {
        ...
        it('should handle complex integrable expression: ∫ (3x^2 + sin(x) - 4e^x) dx = x^3 - cos(x) - 4e^x + C', () => {
            ...
            const expr = op(
                op(
                    op(num(3), 'multiply', op(variable('x'), 'power', num(2))),
                    'add',
                    func('sin', [variable('x')])
                ),
                'subtract',
                op(num(4), 'multiply', new ExponentialNode(variable('x')))
            );
    ...
    describe('integrateDefinite()', () => {
        ...
        it('should integrate constant function from 0 to 5: ∫ 5 dx = 25', () => {
            const expr = num(5);
            expect(integrator.integrateDefinite(expr, 'x', 0, 5, {})).to.be.closeTo(25, tolerance);
        });

        it('should integrate x from 0 to 1: ∫ x dx = 0.5', () => {
            const expr = variable('x');
            expect(integrator.integrateDefinite(expr, 'x', 0, 1, {})).to.be.closeTo(0.5, tolerance);
        });

        it('should integrate x^2 from 0 to 1: ∫ x^2 dx = 1/3', () => {
            const expr = op(variable('x'), 'power', num(2));
            expect(integrator.integrateDefinite(expr, 'x', 0, 1, {})).to.be.closeTo(1/3, tolerance);
        });

        it('should integrate 2x + 3 from 0 to 2: ∫ (2x + 3) dx = 10', () => {
            const expr = op(op(num(2), 'multiply', variable('x')), 'add', num(3));
            expect(integrator.integrateDefinite(expr, 'x', 0, 2, {})).to.be.closeTo(10, tolerance);
        });

        it('should integrate x^3 - 4x + 1 from -1 to 2: ∫ (x^3 - 4x + 1) dx = 0.75', () => {
            const expr = op(
                op(op(variable('x'), 'power', num(3)), 'subtract', op(num(4), 'multiply', variable('x'))),
                'add',
                num(1)
            );
            ...
            // F(2) - F(-1) = -2 - (-2.75) = 0.75
            expect(integrator.integrateDefinite(expr, 'x', -1, 2, {})).to.be.closeTo(0.75, tolerance);
        });

        it('should integrate sin(x) from 0 to PI: ∫ sin(x) dx = 2', () => {
            const expr = func('sin', [variable('x')]);
            expect(integrator.integrateDefinite(expr, 'x', 0, Math.PI, {})).to.be.closeTo(2, tolerance);
        });

        it('should integrate cos(x) from 0 to PI/2: ∫ cos(x) dx = 1', () => {
            const expr = func('cos', [variable('x')]);
            expect(integrator.integrateDefinite(expr, 'x', 0, Math.PI / 2, {})).to.be.closeTo(1, tolerance);
        });

        it('should integrate exp(x) from 0 to 1: ∫ e^x dx = e - 1', () => {
            const expr = new ExponentialNode(variable('x'));
            expect(integrator.integrateDefinite(expr, 'x', 0, 1, {})).to.be.closeTo(Math.E - 1, tolerance);
        });

        it('should integrate 1/x from 1 to e: ∫ 1/x dx = 1', () => {
            const expr = op(num(1), 'divide', variable('x'));
            expect(integrator.integrateDefinite(expr, 'x', 1, Math.E, {})).to.be.closeTo(1, tolerance);
        });

        it('should return 0 when lower and upper bounds are the same', () => {
            const expr = variable('x');
            expect(integrator.integrateDefinite(expr, 'x', 5, 5, {})).to.equal(0);
        });

        it('should handle inverted bounds: ∫ x dx from 5 to 1 = -∫ x dx from 1 to 5 = -12', () => {
            const expr = variable('x');
            // ∫ x dx from 1 to 5 = [x^2/2] from 1 to 5 = 25/2 - 1/2 = 24/2 = 12
            expect(integrator.integrateDefinite(expr, 'x', 5, 1, {})).to.be.closeTo(-12, tolerance);
        });

        it('should integrate x^2 from -1 to 1: ∫ x^2 dx = 2/3', () => {
            const expr = op(variable('x'), 'power', num(2));
            expect(integrator.integrateDefinite(expr, 'x', -1, 1, {})).to.be.closeTo(2/3, tolerance);
        });
        ...
        it('should throw error for singularity within bounds (1/x from -1 to 1)', () => {
            const expr = op(num(1), 'divide', variable('x'));
            expect(() => integrator.integrateDefinite(expr, 'x', -1, 1, {})).to.throw('Division by zero.');
        });

        it('should throw error for domain violation within bounds (ln(x) from -1 to 1)', () => {
            const expr = func('ln', [variable('x')]);
            expect(() => integrator.integrateDefinite(expr, 'x', -1, 1, {})).to.throw('Logarithm of non-positive number.');
        });

        it('should handle pi and e as constants in definite integral', () => {
            // ∫ (pi * x + e) dx from 0 to 1 = [pi*x^2/2 + e*x] from 0 to 1 = pi/2 + e
            const expr = op(op(new ConstantNode('pi'), 'multiply', variable('x')), 'add', new ConstantNode('e'));
            expect(integrator.integrateDefinite(expr, 'x', 0, 1, {})).to.be.closeTo(Math.PI / 2 + Math.E, tolerance);
        });
