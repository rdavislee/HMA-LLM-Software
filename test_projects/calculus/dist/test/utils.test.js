...
describe('ExpressionIntegrator', () => {
    const _checkIndefiniteResult = (expr, variable, expectedIntegratedExpr, expectedUnintegratable, expectedConstant) => {
    ...
    describe('integrateIndefinite()', () => {
        ...
        it('should integrate e^(ax+b) (e.g., e^(3x+2)) using u-sub to (1/a)e^(ax+b) + C', () => {
            ...
            const expr = new expression_1.ExponentialNode(innerExpr); // e^(3x+2)
            const expected = op(op(num(1), 'divide', num(3)), 'multiply', new expression_1.ExponentialNode(innerExpr)); // (1/3)e^(3x+2)
            const expectedWithC = op(expected, 'add', variable('C'));
            _checkIndefiniteResult(expr, 'x', expectedWithC, false, 'C');
        });
        ...
        it('should integrate 2x * e^(x^2) dx using u-sub (u=x^2) to e^(x^2) + C', () => {
            ...
            const expr = op(gPrimeX, 'multiply', new expression_1.ExponentialNode(uExpr)); // 2x * e^(x^2)
            const expected = new expression_1.ExponentialNode(uExpr); // e^(x^2)
            const expectedWithC = op(expected, 'add', variable('C'));
            _checkIndefiniteResult(expr, 'x', expectedWithC, false, 'C');
        });
...
describe('ExpressionIntegrator - U-Substitution', () => {
    const _checkUsubResult = (expr, variable, expectedIntegratedExpr, expectedUnintegratable, expectedConstant) => {
        ...
        if (expectedUnintegratable) {
            ...
            const simplifiedResult = simplifier.simplify(result.integratedExpression);
            const simplifiedExpected = simplifier.simplify(expectedIntegratedExpr);
            (0, chai_1.expect)(simplifiedResult).to.deep.equal(simplifiedExpected);
        }
    };
