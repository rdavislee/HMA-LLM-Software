...
import { AstNode, NodeType, NumberNode, VariableNode, BinaryOpNode, UnaryOpNode, CallNode } from "../types/ast.js";
...
function binaryOp(operator: BinaryOpNode['operator'], left: AstNode, right: AstNode): BinaryOpNode {
    return { type: NodeType.BinaryOperation, operator, left, right };
}

function unaryOp(operator: UnaryOpNode['operator'], operand: AstNode): UnaryOpNode {
...
describe("simplify", () => {
    ...
    describe("Nested Simplifications (assuming recursive simplification)", () => {
        ...
        it("should simplify expressions with multiple rules applied", () => {
            ...
            const input = binaryOp(
                '+',
                binaryOp('*', binaryOp('+', variable('x'), num(0)), num(1)),
                binaryOp('+', sinYSq, cosYSq)
            );
            // Expected: x + 1
