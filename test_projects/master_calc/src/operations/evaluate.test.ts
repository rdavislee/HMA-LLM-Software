...
const num = (value: number): NumberNode => ({ type: NodeType.Number, value });
const variable = (name: string): VariableNode => ({ type: NodeType.Variable, name });
const constant = (name: 'pi' | 'e'): ConstantNode => ({ type: NodeType.Constant, name });
const binary = (operator: '+' | '-' | '*' | '/' | '^', left: AstNode, right: AstNode): BinaryOpNode => ({
    type: NodeType.BinaryOperation,
    operator,
    left,
    right,
});
const unary = (operator: '-', operand: AstNode): UnaryOpNode => ({
    type: NodeType.UnaryOperation,
    operator,
    operand,
});
const call = (functionName: string, args: AstNode[]): CallNode => ({
...
describe("evaluate", () => {
    ...
    it("should evaluate a complex expression with variables and functions", () => {
        ...
        const expression = binary(
            ...
            binary(
                ...
                call("sin", [binary("/", constant("pi"), num(2))])
            ),
            call("log", [num(100)])
        );
    ...
    it("should throw an error for an unsupported AST node type", () => {
        const unknownNode: AstNode = { type: 'UnknownType' as NodeType }; // Cast to simulate unknown type
