import { Token, TokenType } from "../types/tokens.js";
import { AstNode, NumberNode, VariableNode, BinaryOpNode, UnaryOpNode, CallNode, ConstantNode, ExpressionNode, NodeType } from "../types/ast.js";
...
export function parse(tokens: Token[]): ExpressionNode {
    ...
    const consume = (expectedType: TokenType, errorMessage?: string): Token => {
        const token = peek();
    ...
    const functionArities: { [key: string]: number | 'variadic' } = {
    ...
    const parsePrimary = (): ExpressionNode => {
        ...
        switch (token.type) {
            ...
            case TokenType.CONSTANT:
                ...
                node = { type: NodeType.Constant, name: token.value! as 'pi' | 'e' };
                break;
