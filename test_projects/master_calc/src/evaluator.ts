import { evaluate as mathEvaluate, number as mathNumber, create, MathJsInstance, isComplex, isUnit, isBigNumber, isFraction, isMatrix, Fraction } from 'mathjs';
...
function preprocessLog(expression: string): string {
 ...
 */
export function evaluate(expression: string, scope: object = {}): number {
  if (expression.trim() === '') {
    return 0;
  }
  ...
  try {
    ...
    const result = mathEvaluate(preprocessedExpression, scope);
