 ...
 */
export function differentiate(node, variableName) {
    if (variableName === '') {
    ...
    // d/dx(x) = 1, d/dx(y) = 0
        ...
        derivative = node.name === variableName ? num(1) : num(0);
    }
    ...
    // Binary Operations
        ...
        switch (binaryNode.operator) {
            ...
            case '/': // d/dx(u/v) = (u'v - uv') / v^2
                derivative = divide(minus(multiply(uPrime, v), multiply(u, vPrime)), power(v, num(2)));
                break;
            case '^': // Power Rule: d/dx(u^n) = n*u^(n-1)*u'  (assuming n is a constant number)
                if (v.type === NodeType.Number) { // u^n where n is a number
                    ...
                    derivative = multiply(multiply(num(n), power(u, num(n - 1))), uPrimeChain);
                }
                else if (u.type === NodeType.Constant || (u.type === NodeType.Variable && u.name !== variableName) || u.type === NodeType.Number) {
                    // This covers cases like c^f(x) or y^x (where y is a constant w.r.t. x)
                    // d/dx(c^u(x)) = c^u(x) * ln(c) * u'(x)
                    derivative = multiply(multiply(power(u, v), call("ln", [u])), vPrime);
                }
                else {
