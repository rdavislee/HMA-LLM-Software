...
class Parser {
        ...
        while (i < s.length) {
            ...
            switch (char) {
                ...
                case '/':
                    ...
                    break;
                case '^':
                    this.tokens.push({ type: TokenType.POWER });
                    break;
                case '(':
    ...
    parseExpression() {
        ...
        while (this.peek().type === TokenType.PLUS || this.peek().type === TokenType.MINUS) {
            ...
            expr = new expression_1.BinaryOperationExpression(operatorToken.type === TokenType.PLUS ? '+' : '-', expr, right);
        }
        return expr;
    }
    // Term -> Factor ( ( '*' | '/' ) Factor )*
    parseTerm() {
        ...
        while (this.peek().type === TokenType.MULTIPLY || this.peek().type === TokenType.DIVIDE) {
            ...
            expr = new expression_1.BinaryOperationExpression(operatorToken.type === TokenType.MULTIPLY ? '*' : '/', expr, right);
        }
        return expr;
    }
    ...
    parseFactor() {
        ...
        if (this.peek().type === TokenType.POWER) {
            ...
            expr = new expression_1.BinaryOperationExpression('^', expr, right);
        }
        return expr;
    }
    // Unary -> ('+' | '-') Term | Primary
    ...
    parseUnary() {
        if (this.peek().type === TokenType.PLUS || this.peek().type === TokenType.MINUS) {
            ...
            return new expression_1.UnaryOperationExpression(operatorToken.type === TokenType.PLUS ? '+' : '-', operand);
        }
    ...
    parsePrimary() {
        ...
        switch (token.type) {
            case TokenType.NUMBER:
                ...
                return new expression_1.NumberExpression(token.value);
            case TokenType.VARIABLE:
                ...
                return new expression_1.VariableExpression(token.value);
